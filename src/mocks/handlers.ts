...
export const handlers = [
	...
	rest.get(apiEndpoints.GET_MOVIES_PREFERENCES, (req, res, ctx) => {
		...
		return res(
			ctx.status(200),
			ctx.json({
				movies: [...preferencesData.books, ...preferencesData.books, ...preferencesData.books, ...preferencesData.books]
			})
		)
	}),
	rest.get(apiEndpoints.GET_SHOWS_PREFERENCES, (req, res, ctx) => {
		...
		return res(
			ctx.status(200),
			ctx.json({
				shows: [...preferencesData.books, ...preferencesData.books, ...preferencesData.books, ...preferencesData.books]
			})
		)
	}),
